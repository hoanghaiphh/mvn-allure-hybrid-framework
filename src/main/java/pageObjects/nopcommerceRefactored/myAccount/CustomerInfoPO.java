package pageObjects.nopcommerceRefactored.myAccount;

import io.qameta.allure.Step;
import org.openqa.selenium.WebDriver;
import pageObjects.nopcommerceRefactored.MyAccountPO;

import static pageUIs.nopcommerce.myAccount.CustomerInfoPUI.*;

public class CustomerInfoPO extends MyAccountPO {
    private WebDriver driver;

    public CustomerInfoPO(WebDriver driver) {
        super(driver);
        this.driver = driver;
    }

    @Step("Verify that Gender Male radio button was selected")
    public boolean isGenderMaleSelected() {
        return isElementSelected(getVisibleElement(driver, GENDER_MALE_RADIO));
    }

    @Step("Get value in Firstname textbox")
    public String getValueInFirstnameTextbox() {
        return getElementAttributeValue(getVisibleElement(driver, FIRST_NAME_TEXTBOX), "value");
    }

    @Step("Get value in Lastname textbox")
    public String getValueInLastnameTextbox() {
        return getElementAttributeValue(getVisibleElement(driver, LAST_NAME_TEXTBOX), "value");
    }

    @Step("Get value in Company textbox")
    public String getValueInCompanyTextbox() {
        return getElementAttributeValue(getVisibleElement(driver, COMPANY_TEXTBOX), "value");
    }

}
